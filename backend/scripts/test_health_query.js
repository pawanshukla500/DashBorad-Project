    ...
    const query = `
      ...
      WITH order_metrics AS (
        SELECT
          ...
          COUNT(*) FILTER (WHERE
            ( (o.marketplace = 'amazon') AND EXISTS (
               SELECT 1 FROM amazon_settlement_lines a 
               WHERE a.order_id = o.order_id 
                 AND (NULLIF(a.sku, '') IS NULL OR o.sku = a.sku)
            ))
            OR ( (o.marketplace IS NULL OR o.marketplace = 'flipkart') AND EXISTS (
      ...
      return_metrics AS (
        SELECT
          ...
          COUNT(*) FILTER (WHERE
            ( (r.marketplace = 'amazon') AND EXISTS (
               SELECT 1 FROM orders o WHERE o.order_id = r.order_id AND o.sku = r.sku
            ))
            OR ( (r.marketplace IS NULL OR r.marketplace = 'flipkart') AND EXISTS (
